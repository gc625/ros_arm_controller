...
class forward_kin:
  ...
  def callback3(self, data):
    ...
    coords = self.getpos(self.joint3, self.joint4, self.joint1)
    print('Coords: ', coords)
  ...
  def getpos(self, x, y, z):
      xyz = np.array([[2.8 * np.sin(x) * np.sin(z) * np.cos(y) + 2.8 * np.sin(y) * np.cos(z) + 3.2 * np.sin(z) * np.cos(
          x - np.pi / 2)],
  ...
  def makeFk(self, t1, t2, t3):
      xyz = np.array([[2.8*np.sin(t1)*np.sin(t3)*np.cos(t2) + 2.8*np.sin(t2)*np.cos(t3) + 3.2*np.sin(t3)*np.cos(t1 - np.pi/2)],
                      [-2.8*np.sin(t1)*np.cos(t2)*np.cos(t3) + 2.8*np.sin(t2)*np.sin(t3) - 3.2*np.cos(t3)*np.cos(t1 - np.pi/2)],
                      [-3.2*np.sin(t1 - np.pi/2) + 2.8*np.cos(t1)*np.cos(t2) + 4]])
      return xyz
